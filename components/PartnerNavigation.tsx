...
import { LayoutDashboard, PackageSearch, Tag, Wallet } from 'lucide-react';
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1 relative w-16 group">
              {isActive && (
                <motion.div layoutId="mobileNavIndicatorPartner" className="absolute -top-4 w-12 h-1 bg-[#057A42] dark:bg-emerald-500 rounded-full" />
              )}
