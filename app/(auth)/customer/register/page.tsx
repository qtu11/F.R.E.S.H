...
import { Leaf, Lock, Mail, ArrowRight, Eye, EyeOff, User, Phone, MapPin, ArrowLeft } from 'lucide-react';
...
export default function CustomerRegister() {
  ...
  const [rememberMe, setRememberMe] = useState(true); // Mặc định bật để hỗ trợ người dùng tốt nhất
  const router = useRouter();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isVerified || !agreeToTerms) return;
    setError('');
        ...
        <div className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-100 dark:border-slate-800/80 p-10 rounded-[2.5rem] shadow-2xl hover:shadow-emerald-500/10 dark:hover:shadow-emerald-500/5 hover:border-emerald-500/20 dark:hover:border-emerald-500/20 transition-all duration-500">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center gap-2.5 pl-1 select-none">
              ...
              />
              <label htmlFor="rememberMe" className="text-xs text-slate-500 dark:text-slate-400 leading-none cursor-pointer font-bold">
                Ghi nhớ tài khoản trên thiết bị này
              </label>
            </div>

            {/* Checkbox Đồng ý điều khoản và bảo mật */}
            <div className="flex items-start gap-2.5 pl-1 select-none">
              <input 
                type="checkbox" 
                id="agreeTerms"
                checked={agreeToTerms} 
                onChange={e => setAgreeToTerms(e.target.checked)}
                className="w-4 h-4 mt-0.5 rounded border-gray-200 dark:border-slate-800 text-[#057A42] focus:ring-[#057A42] cursor-pointer"
              />
              <label htmlFor="agreeTerms" className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed cursor-pointer font-bold">
                Tôi đồng ý với{' '}
                <Link href="/terms" target="_blank" className="text-[#057A42] dark:text-emerald-400 hover:underline font-extrabold">
                  Điều khoản dịch vụ
                </Link>{' '}
                và{' '}
                <Link href="/privacy" target="_blank" className="text-[#057A42] dark:text-emerald-400 hover:underline font-extrabold">
                  Chính sách bảo mật
                </Link>{' '}
                của F.R.E.S.H.
              </label>
            ...
            <button 
              type="submit" 
              disabled={loading || !isVerified || !agreeToTerms} 
          ...
          <div className="mt-8 pt-8 border-t border-gray-100 dark:border-slate-800 text-center">
            <p className="text-gray-500 dark:text-slate-400 text-sm mb-4">
              ...
              <Link href="/customer/login" className="text-[#057A42] dark:text-emerald-400 font-bold hover:underline">
                Sign In
              </Link>
            </p>
